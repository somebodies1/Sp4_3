...
bool CSpinTower::Create(void)
{
	...
	CSceneNode* cSceneNode = new CSceneNode(CSceneNode::SUPREME,0);
	cSceneNode->SetShader("Shader3D");
	float fCheckHeight = CTerrain::GetInstance()->GetHeight(2.0f, -2.0f);
	cSceneNode->SetWorldTranslateMtx(glm::vec3(-2.0f, fCheckHeight + 1.5f, -2.0f));
	cSceneNode->Init();
	...
	CSceneNode* cSceneNodeChild = new CSceneNode(CSceneNode::AMONGUS, 1);
	cSceneNodeChild->SetShader("Shader3D");
	cSceneNodeChild->Init();
	cSceneNodeChild->SetWorldTranslateMtx(glm::vec3(2.0f, 0.0f, 0.0f));
	...
	CSceneNode* cSceneNodeGrandChild = new CSceneNode(CSceneNode::AMONGUS, 2);
	
	cSceneNodeGrandChild->SetShader("Shader3D");
	cSceneNodeGrandChild->Init();
	cSceneNodeGrandChild->SetWorldTranslateMtx(glm::vec3(-2.0f, 0.0f, 0.0f));
	cSceneNodeGrandChild->bContinuousUpdate = true;
	cSceneNodeGrandChild->SetUpdateRotateMtx(glm::radians(0.5f), glm::vec3(0.0f, 1.0f, 0.0f));
