 ...
 */
Spider::Spider(const glm::vec3 vec3Position,
...
bool Spider::Init(void)
{
	...
	std::string file_path = "Models/Enemy/spider.obj";
	bool success = CLoadOBJ::LoadOBJ(file_path.c_str(), vertices, uvs, normals, true);
	if (!success)
	{
		cout << "Unable to load Models spider" << endl;
		return false;
	}
	...
	iTextureID = CImageLoader::GetInstance()->LoadTextureGetID("Models/Enemy/spider.png", false);
	if (iTextureID == 0)
	{
	...
	m_iWayPointID = cWaypointManager->AddWaypoint(m_iWayPointID, glm::vec3(40.0f, fCheckHeight, 50.0f));
	fCheckHeight = cTerrain->GetHeight(-20.0f, -30.0f);
	m_iWayPointID = cWaypointManager->AddWaypoint(m_iWayPointID, glm::vec3(-20.0f, fCheckHeight, 50.0f));
 ...
 */	
void Spider::SetModel(const glm::mat4 model)
{
	this->model = model;
}
 ...
 */
void Spider::SetView(const glm::mat4 view)
{
	this->view = view;
}
 ...
 */
void Spider::SetProjection(const glm::mat4 projection)
{
 ...
 */
void Spider::AttachCamera(CCamera* cCamera)
{
 ...
 */
bool Spider::IsCameraAttached(void)
{
	if (cCamera)
		return true;
	return false;
 ...
 */
void Spider::SetWeapon(const int iSlot, CWeaponInfo* cWeaponInfo)
{
	if (iSlot == 0)
 ...
 */
CWeaponInfo* Spider::GetWeapon(void) const
{
	if (iCurrentWeapon == 0)
...
/**
 ...
 @param iSlot A const int variable which contains the weapon info to check for. 0 == Primary, 1 == Secondary
 */
void Spider::SetCurrentWeapon(const int iSlot)
{
...
/**
 ...
 @return A bool value
 */
bool Spider::DischargeWeapon(void) const
{
 ...
 */
void Spider::ProcessMovement(const SPIDERMOVEMENT direction, const float deltaTime)
{
 ...
 */
void Spider::ProcessRotate(const float fXOffset)
{
	// Update the yaw
 ...
 */
bool Spider::Update(const double dElapsedTime)
{
 ...
 */
void Spider::UpdateSpiderVectors(void)
{
 ...
 */
void Spider::UpdateFrontAndYaw(void)
{
