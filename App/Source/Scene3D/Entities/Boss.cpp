 ...
 */
Boss::Boss(const glm::vec3 vec3Position,
...
bool Boss::Init(void)
{
	...
	if (!success)
	{
		cout << "Unable to load Models Boss" << endl;
		return false;
	}
 ...
 */	
void Boss::SetModel(const glm::mat4 model)
{
	this->model = model;
}
 ...
 */
void Boss::SetView(const glm::mat4 view)
{
	this->view = view;
}
 ...
 */
void Boss::SetProjection(const glm::mat4 projection)
{
 ...
 */
void Boss::AttachCamera(CCamera* cCamera)
{
 ...
 */
bool Boss::IsCameraAttached(void)
{
	if (cCamera)
		return true;
	return false;
 ...
 */
void Boss::SetWeapon(const int iSlot, CWeaponInfo* cWeaponInfo)
{
	if (iSlot == 0)
 ...
 */
CWeaponInfo* Boss::GetWeapon(void) const
{
	if (iCurrentWeapon == 0)
...
/**
 ...
 @param iSlot A const int variable which contains the weapon info to check for. 0 == Primary, 1 == Secondary
 */
void Boss::SetCurrentWeapon(const int iSlot)
{
...
/**
 ...
 @return A bool value
 */
bool Boss::DischargeWeapon(void) const
{
 ...
 */
void Boss::ProcessMovement(const BOSSMOVEMENT direction, const float deltaTime)
{
	float velocity = fMovementSpeed * deltaTime;
	if (direction == BOSSMOVEMENT::FORWARD)
		vec3Position += vec3Front * velocity;
	if (direction == BOSSMOVEMENT::BACKWARD)
		vec3Position -= vec3Front * velocity;
	if (direction == BOSSMOVEMENT::LEFT)
 ...
 */
void Boss::ProcessRotate(const float fXOffset)
{
	// Update the yaw
 ...
 */
bool Boss::Update(const double dElapsedTime)
{
	...
	case FSM::PATROL:
		...
		else
		{
			...
			ProcessMovement(BOSSMOVEMENT::FORWARD, (float)dElapsedTime);
			if (_DEBUG_FSM == true)
 ...
 */
void Boss::UpdateBossVectors(void)
{
 ...
 */
void Boss::UpdateFrontAndYaw(void)
{
