...
int CProjectileManager::Activate(	glm::vec3 vec3Position, 
									...
									CWeaponInfo::WEAPON_TYPE weaponType)
{
	...
	if (weaponType == CWeaponInfo::WEAPON_TYPE::CARD)
	{
		vProjectile[uiIndexLast]->SetScale(glm::vec3(0.1f, 1.0f, 0.1f));
		vProjectile[uiIndexLast]->SetType(CEntity3D::CARD_PROJECTILE);
	}
	else if (weaponType == CWeaponInfo::WEAPON_TYPE::PISTOL)
	{
