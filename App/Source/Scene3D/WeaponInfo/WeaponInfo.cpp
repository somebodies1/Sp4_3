...
float CWeaponInfo::GetReloadTime(void) const
{
	return dReloadTime;
}

float CWeaponInfo::GetMaxReloadTime(void) const
{
...
bool CWeaponInfo::Update(const double dt)
{
	...
	if (dReloadTime >= 0.0f)
	{
		...
		dReloadTime -= dt;
		//cout << "Reload Time" << dReloadTime << endl;
	...
	if (bFire)
	{
		...
		if (iMagRounds > 0)
		{
			...
			case WEAPON_TYPE::PISTOL:
				CProjectileManager::GetInstance()->Activate(vec3Position + vec3Front * 0.75f,
					vec3Front,
					2.0f, 50.0f, pSource, this->weaponType);
				break;
